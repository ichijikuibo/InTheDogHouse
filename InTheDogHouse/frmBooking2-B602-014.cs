...
namespace InTheDogHouse
{
    public partial class frmBooking2 : Form
    {
        ...
        private void frmBooking2_Load(object sender, EventArgs e)
        {
            ...
            sqlDog = @"select DogNo, dog.BreedNo, Name, CustomerNo, Breed.SizeB from Dog " +
                "join Breed on dog.BreedNo = Breed.BreedNo " +
            ...
            sqlKennelDetails = "SELECT * FROM kennel " +
"WHERE Kennel.SizeK >= @Size order by kennelNo ";
            cmdKennelDetails = new SqlCommand(sqlKennelDetails, conn);
            cmdKennelDetails.Parameters.Add("@Size", SqlDbType.Int);
        ...
        private void txtCustomer_TextChanged(object sender, EventArgs e)
        {
            ...
            if(lstCustomer.Items.Count>0)lstCustomer.SelectedIndex = 0;
        ...
        private void lstDog_Click(object sender, EventArgs e)
        {
            if (lstDog.Items.Count != 0)
            {
                ...
                cmdKennelDetails.Parameters["@Size"].Value = dog["SizeB"];
                ...
                if(filterKennels()==0)
                {
                    MessageBox.Show("There are no kennels available of this size");
                }
            }
        }
        private int filterKennels()
        {
            int numKennels = lstDog.Items.Count;
            DateTime start = dtpStartDate.SelectionStart;
            foreach (DataRow dr in dsInTheDogHouse.Tables["BookedKennels"].Rows)
            {
                DateTime bookedDate = DateTime.Parse(dr["dateStart"].ToString());
                if (start >= bookedDate && start <= bookedDate.AddDays((int)numDays.Value))
                {
                    foreach (DataRow kennel in dsInTheDogHouse.Tables["Kennel"].Rows)
                    {
                        if (kennel["KennelNo"].ToString() == (dr["KennelNo"]).ToString())
                        {
                            dsInTheDogHouse.Tables["Kennel"].Rows.Remove(kennel);
        ...
        private void resetForm()
        {
            ...
            daBooking.Fill(dsInTheDogHouse, "Booking");
            dsInTheDogHouse.Tables["bookingDetail"].Clear();
